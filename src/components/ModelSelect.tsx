    ...
    <div ref={boxRef} className="relative flex items-center gap-1 min-w-0">
      <button
        ...
        className={cn(
          ...
          value
            ? 'bg-ink-950 text-[#f5f1e8] hover:bg-ink-800'
            : 'border border-dashed border-ink-300 text-ink-500 hover:border-signal-600 hover:text-signal-700',
        )}
      ...
      >
        <span
          className={cn(
            ...
            value ? 'bg-signal-600 text-white' : 'bg-ink-950/5',
          )}
        >
          <Cpu size={11} />
        </span>
      ...
      {provider && value && (
        <button
          ...
          className={cn(
            ...
            currentCaps
              ? 'border-ink-200 bg-white/80 text-ink-600 hover:border-signal-600 hover:text-signal-700'
              : 'border-dashed border-amber-500 bg-amber-50 text-amber-700 animate-pulse',
          )}
          title={
            currentCaps
      ...
      {open && provider && (
        <div className="absolute top-full left-0 z-30 mt-2 w-80 max-w-[80vw] animate-rise rounded-2xl border border-ink-200 bg-white p-2.5 shadow-xl">
          ...
          {note && (
            <p className="mt-1.5 truncate px-1 font-mono text-[11px] text-ink-500" title={note}>
              {note}
            </p>
          )}
                ...
                >
                  ...
                  {caps && (
                    <span className="flex items-center gap-1 opacity-70">
                      {caps.supportsVision && (
                        <span title="Vision">
                          <Eye size={10} className="text-signal-600" />
                        </span>
                      )}
                      ...
                      {caps.isReasoning && (
                        <span title="Reasoning">
                          <Brain size={10} className="text-amber-600" />
                        </span>
                      )}
                    </span>
                  )}
