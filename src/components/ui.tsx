  ...
  return (
    <p
      className={cn(
        ...
        tone === 'dark' ? 'text-ink-400' : 'text-ink-500 dark:text-ink-400',
        className,
      )}
    >
      {children}
  ...
  return (
    <button
      className={cn(
        ...
        variant === 'ink' && 'bg-ink-950 text-[#f5f1e8] hover:bg-ink-800 dark:bg-ink-100 dark:text-ink-950 dark:hover:bg-white',
        variant === 'outline' && 'border border-ink-200 bg-white/70 hover:border-ink-400 hover:bg-white dark:border-ink-800 dark:bg-ink-900/80 dark:text-[#ede7db] dark:hover:border-ink-700 dark:hover:bg-ink-900',
        variant === 'ghost' && 'text-ink-700 hover:bg-ink-950/5 dark:text-ink-300 dark:hover:bg-white/5 dark:hover:text-white',
        variant === 'danger-ghost' && 'text-red-700/80 hover:bg-red-50 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-950/30 dark:hover:text-red-300',
        className,
      )}
      {...rest}
    />
...
export function Field(props: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={cn(
        'w-full rounded-lg border border-ink-200 bg-white/80 px-2.5 py-1.5 text-sm outline-none transition placeholder:text-ink-400 focus:border-signal-600 focus:ring-2 focus:ring-signal-600/15 dark:border-ink-800 dark:bg-ink-900/90 dark:text-[#ede7db] dark:placeholder:text-ink-500 dark:focus:border-signal-500 dark:focus:ring-signal-500/20',
        props.className,
      )}
    />
  )
